...
export const getMovieHomeSections = async (req, res) => {
  try {
    ...
    const [
      ...
      Movie.find({ status: 'Active', movieType: { $ne: 'Web Series', $ne: 'Animation' }, ...releasedCondition }).select('title slug originalTitle posterImage releaseYear imdbRating tmdbRating duration runtime genre quality appType isFeatured isTrending').sort({ createdAt: -1 }).limit(15).lean(),
