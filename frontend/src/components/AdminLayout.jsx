    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-[#080312]/95 backdrop-blur-xl text-slate-700 dark:text-slate-300 border-r border-slate-200 dark:border-white/5 w-64 flex-shrink-0">
      ...
      <div className="p-4 border-t border-slate-200 dark:border-white/5">
        <div className="flex items-center gap-3 mb-4">
          <img 
            src={activeUser.profileImage?.startsWith('http') ? activeUser.profileImage : `${import.meta.env.VITE_API_URL || ''}/uploads/avatars/${activeUser.profileImage || 'default.jpg'}`}
            alt={activeUser.name}
            className="w-9 h-9 rounded-full object-cover border border-slate-200 dark:border-slate-800"
          />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{activeUser.name}</p>
            <p className="text-xs text-slate-500 dark:text-slate-400 truncate capitalize">{activeUser.role}</p>
          </div>
        </div>
        <button 
