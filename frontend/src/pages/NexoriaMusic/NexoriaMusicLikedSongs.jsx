...
const NexoriaMusicLikedSongs = () => {
  ...
  return (
    <div className="min-h-full bg-[#0F0F23] text-white">
      ...
      <div className="min-h-[350px] md:min-h-[300px] bg-gradient-to-b from-[#5c42a6] to-[#0F0F23] flex items-end px-4 sm:px-6 pb-4 sm:pb-6 pt-16 md:pt-6 relative z-0">
        <button 
          onClick={() => navigate(-1)}
        ...
        <div className="flex flex-col md:flex-row gap-4 sm:gap-6 items-center md:items-end z-10 relative w-full">
          <div className="w-48 h-48 sm:w-56 sm:h-56 bg-gradient-to-br from-indigo-500 to-purple-800 shadow-2xl flex items-center justify-center rounded-sm overflow-hidden shrink-0 mx-auto md:mx-0">
          ...
          <div className="flex flex-col gap-1 sm:gap-2 min-w-0 flex-1 w-full text-center md:text-left mt-2 md:mt-0">
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black tracking-tighter text-white pb-1 sm:pb-2 drop-shadow-md line-clamp-2 sm:line-clamp-3 w-full">Liked Songs</h1>
          ...
          <>
            ...
            <div className="flex flex-col">
              {tracks.map((track, idx) => (
                <div 
                  key={track._id} 
                  className="grid grid-cols-[40px_1fr_60px] md:grid-cols-[40px_minmax(120px,_4fr)_minmax(120px,_2fr)_80px] gap-2 sm:gap-4 px-2 sm:px-4 py-2 hover:bg-white/10 group transition-colors rounded-md items-center cursor-pointer text-sm font-medium"
                  onClick={() => handlePlay(track, tracks)}
                  onContextMenu={(e) => handleContextMenu(e, track)}
                >
                  <div className="flex items-center justify-end shrink-0">
                    <span className={`text-[#22C55E] group-hover:hidden`}>{idx + 1}</span>
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    ...
                    <div className="flex flex-col min-w-0">
                      <span className={`truncate text-sm ${currentTrack?._id === track._id ? 'text-[#22C55E]' : 'text-white'}`}>{track.title}</span>
                      {track.artist ? (
                        <Link 
                          to={`/nexoria-music/artist/${track.artist._id}`} 
                          className="text-[#94A3B8] hover:underline hover:text-white transition-colors truncate text-xs"
                  ...
                  <div className="hidden md:block min-w-0">
                    {track.album ? (
                      <Link 
                        ...
                        className="text-[#94A3B8] hover:underline hover:text-white transition-colors truncate text-sm block"
                        onClick={(e) => e.stopPropagation()}
                      >
                        {track.album.title}
                      </Link>
                  ...
                  <div className="flex items-center justify-end gap-1 sm:gap-2 text-[#94A3B8]">
                    <button 
                      className={`transition-opacity hover:text-white p-1 ${likedTracks?.includes(track._id) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                      onClick={(e) => {
                        e.stopPropagation();
