...
const NexoriaMusicAlbum = () => {
  ...
  return (
    <div className="min-h-full bg-[#121212] text-white">
      {/* Header Gradient */}
      <div className="h-[30vh] min-h-[300px] bg-gradient-to-b from-[#4A4A4A] to-[#121212] flex items-end px-6 pb-6 relative z-0">
        <button 
          onClick={() => navigate(-1)}
          className="absolute top-6 left-6 w-10 h-10 bg-black/40 hover:bg-black/60 rounded-full flex items-center justify-center transition-colors"
        >
          <ArrowLeft className="w-6 h-6" />
        </button>
        
        <div className="flex gap-6 items-end z-10 relative">
          <div className="w-48 h-48 sm:w-56 sm:h-56 bg-[#282828] shadow-2xl flex items-center justify-center rounded-sm overflow-hidden shrink-0">
            {album.coverImage ? (
      ...
      <div className="px-6 py-6 flex items-center justify-between relative z-10 bg-black/10 backdrop-blur-sm">
      ...
      <div className="px-6 pb-20 relative z-10">
        {tracks.length === 0 ? (
          ...
          <>
            {/* Table Header */}
            <div className="grid grid-cols-[32px_minmax(120px,_4fr)_minmax(120px,_1fr)] gap-4 px-4 py-2 text-sm text-[#a7a7a7] border-b border-white/10 mb-4 sticky top-16 bg-[#121212] z-10 uppercase tracking-widest font-medium">
              ...
              {tracks.map((track, idx) => (
                ...
                >
                  ...
                  <div className="flex flex-col truncate">
                    ...
                    {track.artist && <span className="text-[#a7a7a7] group-hover:text-white transition-colors truncate">{track.artist.name}</span>}
                  </div>
