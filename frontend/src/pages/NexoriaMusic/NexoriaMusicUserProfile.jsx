...
const NexoriaMusicUserProfile = () => {
    ...
    return (
      <div className="min-h-full bg-[#121212] flex items-center justify-center text-white">
        <div className="animate-pulse flex flex-col items-center">
           <div className="w-48 h-48 bg-zinc-800 rounded-full mb-4"></div>
  ...
  return (
    <div className="min-h-full bg-[#121212] text-white relative pb-32">
      {/* Background Gradient */}
      <div className="absolute top-0 left-0 right-0 h-[400px] bg-gradient-to-b from-[#535353] to-[#121212] pointer-events-none z-0 opacity-80" />
      ...
      <div className="relative z-10 px-4 sm:px-8 pt-16 sm:pt-24 max-w-[1920px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-end gap-6 mb-12">
          <div className="w-[192px] h-[192px] sm:w-[232px] sm:h-[232px] rounded-full shadow-[0_4px_60px_rgba(0,0,0,0.5)] overflow-hidden bg-zinc-800 flex items-center justify-center shrink-0">
            {user.avatar ? (
        ...
        <div className="bg-black/20 rounded-xl p-4 sm:p-6 backdrop-blur-md">
               ...
               <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-6">
                 {playlists.map(playlist => (
                    <div 
                      ...
                      className="p-4 bg-[#181818] hover:bg-[#282828] rounded-md transition-colors duration-300 cursor-pointer group flex flex-col"
                    >
                      <div className="w-full aspect-square bg-zinc-800 rounded-md mb-4 overflow-hidden shadow-[0_8px_24px_rgba(0,0,0,0.5)] relative">
                        {playlist.coverImage ? (
                      ...
                      <p className="text-sm text-[#a7a7a7] line-clamp-2 font-medium">By {playlist.creator?.name || 'User'}</p>
                    </div>
                 ))}
               </div>
             </section>
           ...
           {recentlyPlayed.length > 0 && (
             <section>
               ...
               <div className="flex flex-col">
                 <div className="grid grid-cols-[16px_minmax(120px,1fr)_1fr_minmax(120px,1fr)] sm:grid-cols-[16px_minmax(120px,2fr)_minmax(120px,1.5fr)_minmax(120px,1fr)] gap-4 px-4 py-2 text-[#a7a7a7] text-sm border-b border-white/10 mb-2">
                 ...
                 {recentlyPlayed.map((track, index) => {
                   ...
                   return (
                     ...
                     >
                       <div className="text-center text-[#a7a7a7] text-base font-medium group-hover:hidden">
                       ...
                       <div className="flex items-center gap-3 overflow-hidden">
                         <div className="w-10 h-10 bg-zinc-800 shrink-0">
                         ...
                         <div className="flex flex-col justify-center overflow-hidden">
                           <span className={`text-base font-normal truncate ${isPlayingCurrent ? 'text-[#1ed760]' : 'text-white'}`}>{track.title}</span>
                           <span className="text-sm text-[#a7a7a7] truncate hover:underline cursor-pointer" onClick={(e) => { e.stopPropagation(); navigate(`/nexoria-music/artist/${track.artist?._id}`); }}>
                             {track.artist?.name || 'Unknown Artist'}
                           </span>
                         </div>
                       </div>

                       <div className="text-sm text-[#a7a7a7] font-normal truncate hidden sm:block hover:underline cursor-pointer" onClick={(e) => { e.stopPropagation(); if (track.album) navigate(`/nexoria-music/album/${track.album._id}`); }}>
                         {track.album?.title || ''}
                       </div>

                       <div className="flex items-center justify-end text-[#a7a7a7] text-sm tabular-nums">
