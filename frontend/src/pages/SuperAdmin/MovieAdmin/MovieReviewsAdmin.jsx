...
export default function MovieReviewsAdmin({ type = 'Movie' }) {
  const { data: res, isLoading } = useGetAdminMovieReviewsQuery();
  const [moderateReview] = useModerateMovieReviewMutation();
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            ...
            {type === 'Web Series' ? 'TV Show' : type} Reviews
          </h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">
            Approve, hide, or delete user reviews.
          </p>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-white/10">
                {reviews.length === 0 ? (
                  ...
                  <tr key={review._id} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.02]">
                    <td className="p-4">
                      <div className="font-semibold text-slate-900 dark:text-white">{review.user?.name || 'Unknown'}</div>
                      <div className="text-xs text-slate-500 dark:text-slate-400">{review.user?.email}</div>
                    </td>
                    <td className="p-4 text-slate-700 dark:text-slate-300 font-medium">
                      {review.movie?.title || 'Deleted Movie'}
                    </td>
