...
import { useGetPurchaseRequestsQuery, useApprovePurchaseRequestMutation, useRejectPurchaseRequestMutation } from '../../features/api/paymentApiSlice';
...
const PurchaseRequests = () => {
  const { data: requestsRes, isLoading } = useGetPurchaseRequestsQuery();
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <Helmet>
        <title>Purchase Requests - Super Admin</title>
      </Helmet>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold dark:text-white flex items-center gap-2">
              ...
              App & Game Purchase Requests
            </h1>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Review and manage app & game payments</p>
          </div>
        </div>
      </div>
              ...
              {requests.map(req => (
                <tr key={req._id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="p-4 text-slate-700 dark:text-slate-300 font-medium">
                    {req.post?.title || req.game?.title || 'Unknown Item'}
                    {req.game && <span className="ml-2 text-[10px] bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded-full">Game</span>}
                  </td>
              ...
              {requests.length === 0 && (
                <tr>
                  <td colSpan="7" className="p-8 text-center text-slate-500">
                    ...
                    No app purchase requests found.
                  </td>
                </tr>
              )}
            </tbody>
