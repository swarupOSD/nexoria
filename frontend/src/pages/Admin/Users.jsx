...
import { Search, Filter, Trash2, Eye, Shield, X, Star, Crown, ShieldAlert, AlertTriangle, FileText, Lock, Send, Mail } from 'lucide-react';
import { 
  useGetUsersQuery, 
...
const AdminUsers = () => {
  ...
  const handleModerationSubmit = async (e) => {
    ...
    try {
      ...
      alert('Error applying action: ' + (error?.data?.message || 'Server error'));
    }
  };
  ...
  const handleSendMessage = async (e) => {
    ...
    const recipientId = messageRecipientId;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setIsMessageModalOpen(true)} className="premium-btn gap-2 py-2">
      ...
      >
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row gap-4 items-center justify-between">
          <div className="flex gap-4 w-full lg:w-auto">
            ...
            <select 
              ...
              className="px-4 py-2 bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all text-sm dark:text-white"
            >
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">
                <th className="p-4 w-10"><input type="checkbox" onChange={selectAll} checked={selectedUsersIds.length === filteredUsers.length && filteredUsers.length > 0} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500" /></th>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {isLoading ? (
                ...
                <tr key={user._id} className={`hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors ${selectedUsersIds.includes(user._id) ? 'bg-blue-50/30 dark:bg-blue-900/10' : ''}`}>
                  <td className="p-4">
                    <input type="checkbox" checked={selectedUsersIds.includes(user._id)} onChange={() => toggleSelectUser(user._id)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                  </td>
                  <td className="p-4">
                ...
                {moderationAction !== 'restrict' ? (
                  <>
                    <div>
                      ...
                      <textarea name="reason" required placeholder={`Reason for ${moderationAction}...`} className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 h-24 resize-none"></textarea>
                    </div>
                    ...
                    {(moderationAction === 'suspend' || moderationAction === 'ban') && (
                      <div>
                        ...
                        <select name="days" className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
                  ...
                  <div className="space-y-3">
                    ...
                    <label className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg cursor-pointer">
                      <input type="checkbox" name="disableCommenting" defaultChecked={selectedUser.restrictions?.disableCommenting} className="w-5 h-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                      <span className="text-sm font-semibold dark:text-slate-300">Disable Commenting</span>
                    </label>
                    <label className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg cursor-pointer">
                      <input type="checkbox" name="disableRatings" defaultChecked={selectedUser.restrictions?.disableRatings} className="w-5 h-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                      <span className="text-sm font-semibold dark:text-slate-300">Disable Ratings</span>
                    </label>
                    <label className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg cursor-pointer">
                      <input type="checkbox" name="disableUploads" defaultChecked={selectedUser.restrictions?.disableUploads} className="w-5 h-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
        ...
        {isNoteModalOpen && selectedUser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="glass-card relative w-full max-w-md p-6 shadow-2xl z-10 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
              ...
              <form onSubmit={handleNoteSubmit} className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                <label className="block text-sm font-semibold mb-2 dark:text-slate-300">Add New Note</label>
                <textarea name="note" required placeholder="Type internal note here..." className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 h-20 resize-none mb-3"></textarea>
                <button type="submit" className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold transition">Add Note</button>
              </form>
            </motion.div>
          </div>
        )}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="glass-card relative w-full max-w-md p-6 shadow-2xl z-10 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
              ...
              <form onSubmit={async (e) => {
                ...
                <div>
                  ...
                  <select name="action" defaultValue={selectedUser.isPremium ? "grant" : "grant"} className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-yellow-500">
                ...
                <div>
                  ...
                  <select name="premiumType" defaultValue={selectedUser.premiumType || '30 Days'} className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-yellow-500">
        ...
        {isMessageModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="glass-card relative w-full max-w-md p-6 shadow-2xl z-10 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-visible">
              ...
              <form onSubmit={handleSendMessage} className="space-y-4 relative">
                <div>
                  <label className="block text-sm font-semibold mb-2 dark:text-slate-300">Select User</label>
                  <div className="relative group">
                    <select 
                      required
                      value={messageRecipientId}
                      onChange={(e) => setMessageRecipientId(e.target.value)}
                      className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary appearance-none cursor-pointer"
                    >
                    ...
                    {messageRecipientId && (
                      <img 
                        src={mockUsers.find(u => u._id === messageRecipientId)?.profileImage || '/default.jpg'} 
                        alt="selected user" 
                        className="absolute left-3 top-2.5 w-7 h-7 rounded-full object-cover pointer-events-none"
                      />
                    )}
                    {!messageRecipientId && (
                ...
                <div>
                  ...
                  <textarea 
                    name="message" 
                    required 
                    placeholder="Write a personal message to this user..." 
                    className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary h-28 resize-none"
                  ></textarea>
                </div>
